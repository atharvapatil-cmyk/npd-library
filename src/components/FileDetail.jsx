          ...
          <div style={{ marginBottom:20 }}>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:8 }}>
              <div style={{ padding:12, borderRadius:10, background:'#f0fdf4', border:'1px solid #bbf7d0' }}>
                <div style={{ fontSize:10, fontWeight:700, color:'#16a34a', marginBottom:6 }}>CAN VIEW</div>
                {folder?.viewRoles.map(r => (
                  <div key={r} style={{ fontSize:10, color:'#166534', marginBottom:2 }}>&bull; {ROLE_META[r]?.label}</div>
                ))}
              </div>
              <div style={{ padding:12, borderRadius:10, background:'#eff6ff', border:'1px solid #bfdbfe' }}>
                <div style={{ fontSize:10, fontWeight:700, color:'#2563eb', marginBottom:6 }}>CAN EDIT</div>
                {folder?.editRoles.map(r => (
                  <div key={r} style={{ fontSize:10, color:'#1e40af', marginBottom:2 }}>&bull; {ROLE_META[r]?.label}</div>
                ))}
              </div>
            </div>
          </div>
              ...
              {fakeVersions.map((v, i) => (
                <div key={i} style={{ display:'flex', alignItems:'center', gap:10, padding:'10px 14px', borderRadius:10, background:'var(--bg-surface)', border:'1px solid var(--border)' }}>
                  ...
                  <div style={{ flex:1 }}>
                    ...
                    <div style={{ fontSize:11, color:'var(--text-muted)' }}>{v.by} &middot; {formatDate(v.date)}</div>
                  </div>
