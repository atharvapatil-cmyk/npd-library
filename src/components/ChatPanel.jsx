...
const QUICK_REPLIES = [
  { label: 'ð¬ Formulation docs', q: 'formulation docs' },
  { label: 'ð Stability studies', q: 'stability studies' },
  { label: 'ð All sections',      q: 'list all sections' },
  { label: 'âï¸ Regulatory files',  q: 'regulatory files' },
  { label: 'ð·ï¸ Search by tag',     q: 'search by tag' },
  { label: 'ð¦ Batch records',      q: 'batch records' },
];

const LIB_LINES = [
  "Oh that? Found it in 0.3 seconds. ð",
  "I've memorised every file in this vault â ask me ANYTHING.",
  "My filing system is immaculate. Unlike some people's desks. ð",
  "Here's what I found! (I'm basically a supercomputer in glasses)",
  "YOOO I found it!! ð",
  "Searching the depths of the NPD vault...",
  "Found it! Also you look great today. ðâ¨",
];
...
function getSearchResults(q, sections, user, matrix) {
  ...
  if (query.includes('formul')) {
    ...
    return { text: `Found **${r.length} formulation files** you can access! ð§ª`, files: r.slice(0,5) };
  }
  if (query.includes('stabil')) {
    const r = allFiles.filter(f => f.name.toLowerCase().includes('stab') || (f.tags||[]).includes('stability'));
    return { text: `Here are the **stability study files** â science is cool ð¬`, files: r.slice(0,5) };
  }
  if (query.includes('list') || query.includes('section')) {
    return { text: `The library has **${sections.length} sections**: ${sections.map(s => s.name).join(', ')}. Click any section in the sidebar to jump there! ð` };
  }
  if (query.includes('reg') || query.includes('fssai') || query.includes('fda') || query.includes('ayush') || query.includes('compliance')) {
    ...
    return { text: `**Regulatory files** incoming! âï¸ Found ${r.length} docs:`, files: r.slice(0,5) };
  }
  if (query.includes('batch') || query.includes('trial')) {
    ...
    return { text: `**Batch & trial records** â ${r.length} found! ð`, files: r.slice(0,5) };
  }
  if (query.includes('tag')) {
    return { text: `Tag search: just type a tag like **ayurvedic**, **collagen**, **fssai**, **pilot**, **sop**... I'll find it! ð·ï¸` };
  }
  if (query.includes('sop')) {
    const r = allFiles.filter(f => (f.tags||[]).includes('sop') || f.name.toLowerCase().includes('sop'));
    return { text: `SOPs found! Standard Operating Procedures â ${r.length} docs ð`, files: r.slice(0,5) };
  }
  if (query.includes('coa') || query.includes('certificate')) {
    ...
    return { text: `COA files! Found ${r.length} certificates of analysis ð§¾`, files: r.slice(0,5) };
  }
  ...
  return {
    text: `Hmm, couldn't find anything for "${q}" ð¤ Try: **formulation**, **COA**, **SOP**, **trial**, **stability**, **label**, **collagen**... or any tag! I promise I know everything in here. ð`
  };
}
  ...
  return (
    <div style={{ width: size, height: size, flexShrink: 0 }}>
      <svg viewBox="0 0 64 64" fill="none" xmlns="http://www.w3.org/2000/svg" style={{ width:'100%', height:'100%' }}>
        ...
        <text x="32" y="49.5" textAnchor="middle" fill="#16a34a" fontSize="4" fontWeight="800" fontFamily="system-ui,sans-serif">ð LIB</text>
      </svg>
    </div>
  );
}
  ...
  return (
    <div className="chat-result-card">
      ...
      <div className="chat-result-file-info">
        ...
        <div className="chat-result-file-path">{file.section} âº {file.folder}</div>
        {(file.tags||[]).length > 0 && (
...
export default function ChatPanel({ onClose, currentUser, accessMatrix, sections }) {
  const [messages, setMessages] = useState([{
    role: 'bot',
    text: `HEY ${currentUser?.name?.split(' ')[0].toUpperCase()}!! ð I'm **Lib** â your NPD Document Librarian. I know every. single. file. in this place. Ask me by name, tag, topic, anything. Let's GO! ð`,
  }]);
    ...
    <div className="chat-float-panel">
      ...
      <div className="chat-float-header">
        <div style={{ display:'flex', alignItems:'center', gap:'10px' }}>
          ...
          <div>
            <div className="chat-float-name">Lib â¨</div>
            <div className="chat-float-status">Document Librarian Â· Always on ð¢</div>
          </div>
        </div>
        <button className="chat-float-close" onClick={onClose}>â</button>
      </div>

      {/* Quick replies */}
      <div className="chat-quick-bar">
        {QUICK_REPLIES.map(r => (
          <button key={r.q} className="chat-quick-chip" onClick={() => send(r.q)}>{r.label}</button>
        ))}
      </div>
